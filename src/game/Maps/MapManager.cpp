...
void MapManager::Update(uint32 diff)
{
    ...
    uint32 updateStep = verySlowWorld ? 9 : slowWorld ? 6 : sWorld.GetActiveSessionCount() ? 3 : 2;

    for (auto& map : i_maps)
    {
        bool full = mapUpdateStep < updateStep && (map.second->HasRealPlayers() || mapUpdateStep == 0);
        if (m_updater.activated())
            m_updater.schedule_update(new MapUpdateWorker(*map.second, uint32(full ? i_timer.GetCurrent() : 0), diff, m_updater));
        else
            map.second->Update(uint32(full ? i_timer.GetCurrent() : 0), diff);
    }

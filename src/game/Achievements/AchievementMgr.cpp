    ...
    switch (dataType)
    {
        ...
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_DRUNK:
         ...
         case ACHIEVEMENT_CRITERIA_DATA_TYPE_BG_LOSS_TEAM_SCORE:
         {
             ...
             uint32 score = bg->m_teamScores[source->GetTeamId() == TEAM_INDEX_ALLIANCE ? HORDE : ALLIANCE];
        ...
        // }
         case ACHIEVEMENT_CRITERIA_DATA_TYPE_BG_TEAMS_SCORES:
         {
             ...
             uint32 winnnerScore = bg->m_teamScores[winnerTeam];
             uint32 loserScore = bg->m_teamScores[BattleGroundWinner(!uint32(winnerTeam))];
...
void AchievementMgr::SendAchievementEarned(AchievementEntry const* achievement) const
{
    ...
    std::string title = achievementEntry && achievementEntry->name[0] ? achievementEntry->name[0] : "";
...
void AchievementMgr::SetCriteriaProgress(AchievementCriteriaEntry const* entry, uint32 changeValue, ProgressType ptype)
{
    ...
    if (achievement && achievement->name[0]) {
        ...
        sLog.outDebug("AchievementMgr::SetCriteriaProgress(%u) %s criteria(%s) for %s[%s], %u points (%s)", entry->ID, m_player->GetName(), entry->name[0], breadCrumbs.c_str(), achievement->name[0], achievement->points, achievement->description[0]);
    }
...
void AchievementMgr::CompletedAchievement(AchievementEntry const* achievement)
{
    ...
    if (achievement && achievement->name[0]) {
        ...
        sLog.outDebug("AchievementMgr::CompletedAchievement(%u) %s %s[%s], %u points (%s)", achievement->ID, m_player->GetName(), breadCrumbs.c_str(), achievement->name[0], achievement->points, achievement->description[0]);
    }
...
bool AchievementGlobalMgr::hasAchiever(WorldSession* session) const
{
    ...
    return plr->GetAchievementMgr().HasAchiever();
